...
def readCSV (filename):
    return pd.read_csv(path+filename+'.csv', delimiter=';', sep='delimiter')
...
def read_data():
    ...
    # train data
    train_data = readCSV('train')
    test_data = readCSV('test')
    users_data = readCSV('users')
    products_data = readCSV('products')
